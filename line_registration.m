function line_registration()
    ...
    for idx = 1:length(lineCounts)
        ...
        % Map the original and fitted start/end points to 4D spherical space
        [pointsStart, pointsEnd, furthestPointsStart, furthestPointsEnd] = ...
            mapPointsToSpherical(L, noiseFreeStartEndPoints, furthest_points);
        
        % Form spherical lines (via wedge product) and normalize them
        [sphericalLines, sphericalLinesNoise] = computeSphericalLines(pointsStart, pointsEnd, ...
                                                                    furthestPointsStart, furthestPointsEnd);
